...
namespace Logic
{
        ...
        public Dictionary<TokenColor, int> colorScoreMulti = new Dictionary<TokenColor, int>()
    {
        ...
        protected override void GridChanged(Token tokenChanged)
        {
            ...
            if (tokenGroup.Count >= groupCollapseNum)
            {
                ...
                // Buncha: pause before destroys so the placed tile can land first
    ...
    public class Game
    {
        ...
        public void SetupNearEndGameDebug()
        {
            ...
            Vector2Int leftPos = mid + Vector2Int.left;
            Vector2Int rightPos = mid + Vector2Int.right;
            if (!grid.HasTile(mid) || !grid.HasTile(leftPos) || !grid.HasTile(rightPos))
            {
                mid = new Vector2Int(1, 0);
                leftPos = mid + Vector2Int.left;
                rightPos = mid + Vector2Int.right;
            }

            Vector2Int emptyPos = mid + Vector2Int.up;
            if (!grid.HasTile(emptyPos) || emptyPos == leftPos || emptyPos == mid || emptyPos == rightPos)
            {
                emptyPos = mid + Vector2Int.down;
            }
            if (!grid.HasTile(emptyPos) || emptyPos == leftPos || emptyPos == mid || emptyPos == rightPos)
            {
                emptyPos = new Vector2Int(0, 0);
            }
            if (emptyPos == leftPos || emptyPos == mid || emptyPos == rightPos)
            {
                emptyPos = new Vector2Int(gridSize.x - 1, gridSize.y - 1);
            }
